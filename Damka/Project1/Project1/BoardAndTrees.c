...
SingleSourceMovesTreeNode* captureMove(SingleSourceMovesTreeNode* moveNode, checkersPos* capturedPlayer, checkersPos* capturePos, checkersPos* src, Board board, int totalCaptures, int playOptionIndex)
{
	...
	duplicateBoard(moveNode->board, movementToAdd); 
...
void freeMoveTree(SingleSourceMovesTree* tree) // free MoveTree function
{
	if (tree == NULL) 
		return;
...
void freeMoveTreeNodeHelper(SingleSourceMovesTreeNode* source) // free MoveTree function helper
{
	if (source == NULL)
		return;
